...
@endif
 ...
 <!--<form action="{{url('panel/sessions/store')}}" method="post">-->
 <!--     @csrf-->
    <div id="collapseSession<?php echo e(!empty($session) ? $session->id :'record'); ?>" aria-labelledby="session_<?php echo e(!empty($session) ? $session->id :'record'); ?>" class=" collapse <?php if(empty($session)): ?> show <?php endif; ?>" role="tabpanel">
        <div class="panel-collapse text-gray">
            <div class="js-content-form session-form" data-action="/panel/sessions/store">
                <!--<input type="text" name="ajax[<?php echo e(!empty($session) ? $session->id : 'new'); ?>][webinar_id]" value="<?php echo e(!empty($webinar) ? $webinar->id :''); ?>">-->
                <!--<input type="text" name="ajax[<?php echo e(!empty($session) ? $session->id : 'new'); ?>][chapter_id]" value="<?php echo e(!empty($chapter) ? $chapter->id :''); ?>" class="chapter-input">-->
                    ...
                    <div class="col-12 col-lg-6">
                        ...
                        <div class="form-group">
                            ...
                            <div class="js-session-api">
                                ...
                                <div class="custom-control custom-radio custom-control-inline bigBlueButton12">
                        ...
                        <div class="form-group js-local-link <?php echo e((!empty($session) and $session->session_api == 'agora') ? 'd-none' : ''); ?>">
                            ...
                            <div class="invalid-feedback"></div>
                             @if($errors->has('getJoinLink'))
                                <div class="error">{{ $errors->first('getJoinLink') }}</div>
                                 @endif
                        </div>
                        ...
                        $webChapter = DB::table('webinar_chapters')->where('user_id', Auth::user()->id)->pluck('webinar_id')->unique();
                        $webinar = \App\Models\Webinar::where('teacher_id', Auth::user()->id)->whereIn('id',$webChapter)->where('type','webinar')->get();
                        //dd($webinar,$webChapter,count($webinar),count($webChapter));
                        
                        ?>
                       
                         <div class="form-group">
                            <label class="input-label"><?php echo e(trans('public.course_page_title')); ?></label> 
                            <select multiple="multiple" class="form-control" name="ajax[<?php echo e(!empty($session) ? $session->id : 'new'); ?>][webinar_id][]">
                                  <option value="" disabled>
                                  Please select course
                                </option>
                                @foreach($webinar as $webinar_data)
                              
                                <option value="{{$webinar_data->id}}">
                                    {{$webinar_data->title}}
                                </option>
                             ...
                             @if($errors->has('webinar_id'))
                                <div class="error">{{ $errors->first('webinar_id') }}</div>
                                 @endif
                        </div>
           ...
           <!--         <button type="submit" class="js-save-session btn btn-sm btn-primary"><?php echo e(trans('public.save')); ?></button>-->
           <!--<?php if(!empty($session)): ?>-->
...
<?php /**PATH /home/livedivine/public_html/resources/views/web/default/panel/webinar/create_includes/accordions/session.blade.php ENDPATH**/ ?>

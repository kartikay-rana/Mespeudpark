...
use App\User;
use App\Imports\ImportQuizzesQuestion;
...
class QuizController extends Controller
{
    ...
    { //dd($request->all());
        ...
        if (!empty($webinar)) {
            ...
            if ($request->quiz_pdf) {
                $folder = 'uploads/pdf';
                $file = time() . "." . $request->quiz_pdf->extension();
                $request->quiz_pdf->move(public_path($folder), $file);
                $file_name  = get_base_url() . '/' . $folder . '/' . $file;
            }
            ...
            $quiz = Quiz::create([
                ...
                'quiz_pdf' =>  $file_name ?? null,
                'q_type' =>  $data['q_type'] ?? null,
                'negative_mark' => $data['negative_mark'] ?? null,
    ...
    public function edit(Request $request, $id)
    {
        ...
        //dd($quiz->webinar_id);
        $webinar1 = Webinar::where('id',$quiz->webinar_id)->first();
    ...
    {   //dd($request->all());
        ...
        $quiz->update([
            ...
            'chapter_id' => !empty($chapter) ? $chapter->id : null,
            'chapter_section_id' => !empty($request->chapter_id) ? json_encode($request->chapter_id) : null,
        ...
        //      foreach($request->chapter_id as $key=>$val){
       
        //     $sectionAddMultiple = DB::table('quiz_section')->insert(
        //         [ 'quiz_id' =>$id,
        //           'chapter_id'=>$val
        //             ]
        //         );
    ...
    public function fileImport(Request $request,$quizeId)
    {  
        //dd($request->all());
        // Excel::import(new ImportProduct, $request->file('file')->store('temp'));
    
       return view('admin.quizzes.importQuestion',compact('quizeId'));
    }
    public function fileImportPost(Request $request)
    {  
        // dd($request->all());
        Excel::import(new ImportQuizzesQuestion, $request->file('file')->store('temp'));
        $toastData = [

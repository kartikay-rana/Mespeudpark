...
class SessionController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        foreach($data['webinar_id'] as $key=>$w_id){
            //dd(count($data['webinar_id']) , $key+1);
            $webinar = Webinar::find($w_id);
         $WebinarChapter_data=WebinarChapter::where('webinar_id',$w_id)->first();
       // dd($WebinarChapter_data); 
        if($WebinarChapter_data == null){
            $toastData = [
           // 'title' => trans('public.request_failed'),
            'msg' => $webinar->slug.''.'section not  create',
            'status' => 'success'
        ];
       return redirect()->back()->with(['toast' => $toastData]);
        ...
        if (!empty($webinar) and $webinar->canAccess($user)) {
            ...
            if ($sessionDate->getTimestamp() < $webinar->start_date) {
                ...
                'creator_id' => $user->id,
                'webinar_id' => $w_id,
                'chapter_id' => $WebinarChapter_data->id,
            ...
            if(count($data['webinar_id']) == $key+1){
               // dd(count($data['webinar_id']) == $key+1);
             ...
             //return redirect('panel/meetings/create_new')->with('message',"Successfully create");
    ...
    private function handleBigBlueButtonApi($session, $user)
    {
        $createMeeting = \Bigbluebutton::initCreateMeeting([
            ...
            'record'=>true,
            'logoutUrl' => 'https://livedivine.in/login',
        ]);

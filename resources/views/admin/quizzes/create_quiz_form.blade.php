<div data-action="{{ !empty($quiz) ? '/admin/quizzes/'. $quiz->id .'/update' : '/admin/quizzes/store' }}" enctype="multipart/form-data" class="js-content-form quiz-form webinar-form">
    {{ csrf_field() }}
    <section>
        ...
        <div class="row">
            <div class="col-12 col-md-4">
                ...
                <div class="form-group">
                    ...
                    {{-- <input type="text" value="{{ old('type') }}" name="type" class="form-control @error('type')  is-invalid @enderror" placeholder=""/> --}}
                    <select class="form-control" name="q_type">


                        <option value="{{'null'}}" {{  (!empty($quiz) and $quiz->q_type == '') ? 'selected' : '' }}>Null</option>
                        <option value="{{'c_a_q'}}" {{  (!empty($quiz) and $quiz->q_type == 'c_a_q') ? 'selected' : '' }}>Current affair Question</option>
                        <option value="{{'p_y_q'}}" {{  (!empty($quiz) and $quiz->q_type == 'p_y_q') ? 'selected' : '' }}>Priveris Year Question</option>
                        <option value="{{'section'}}" {{  (!empty($quiz) and $quiz->q_type == 'section') ? 'selected' : '' }}>Section time set</option>
                    </select>
                    @error('q_type')
                    <div class="invalid-feedback">
                        {{ $message }}
                ...
                {{-- <div class="form-group">
                    ...
                    <input type="file" value="{{ old('quiz_pdf') }}" name="quiz_pdf" class="form-control"/>
                    @error('quiz_pdf')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
                ...
                <div class="form-group">
                    <label class="input-label">{{ 'Negative Mark' }}</label>
                    <input type="number" value="{{ !empty($quiz) ? $quiz->negative_mark : old('negative_mark') }}" name="ajax[negative_mark]" class="form-control @error('negative_mark')  is-invalid @enderror" placeholder=""/>
                    @error('negative_mark')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
    ...
    @if(!empty($quiz))
        <section class="mt-5">
            <div class="d-flex justify-content-between align-items-center pb-20">
                ...
                <a href="{{route('quizImport',$quiz->id)}}"><button type="button" class="btn btn-primary btn-sm ml-2 mt-3" >
                  Question Import
                </button></a>
...
@if(!empty($quiz))
     @include('admin.quizzes.modals.import_question')
